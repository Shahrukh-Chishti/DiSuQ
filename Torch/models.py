...
def box4Branches(Rep,basis,Ej,Ec,El,sparse=SPARSE):
...
def zeroPi(Rep,basis,Ej=10.,Ec=50.,El=10.,EcJ=100.,sparse=SPARSE,symmetry=False,_L_=(L_,L0),_C_=(C_,C0),_J_=(J_,J0),_CJ_=(4*C_,4*C0),device=None):
...
def prismon(Rep,basis,Ej=10.,Ec=50.,El=10.,EcJ=100.,sparse=SPARSE,symmetry=False,_L_=(L_,L0),_C_=(C_,C0),_J_=(J_,J0),_CJ_=(4*C_,4*C0)):
    ...
    control_iD = ('La','Lb','Lc')
    circuit = Rep(circuit,control_iD,basis,sparse,pairs)
    return circuit

def transmon(Rep,basis,Ej=10.,Ec=0.3,sparse=SPARSE):
    transmon = [J(0,1,Ej,'J')]
    transmon += [C(0,1,Ec,'C')]
    control_iD = ()
...
def splitTransmon(Rep,basis,sparse=SPARSE):
...
def oscillatorLC(Rep,basis,El=.00031,Ec=51.6256,sparse=SPARSE):
    circuit = [L(0,1,El,'L'),C(0,1,Ec,'C')]
    control_iD = ()
    return Rep(circuit,control_iD,basis,sparse)

def fluxoniumArray(Rep,basis,shunt=None,gamma=1.5,N=0,Ec=100,Ej=150,sparse=SPARSE):
    # N : number of islands
...
def fluxonium(Rep,basis,El=.0003,Ec=.1,Ej=20,sparse=SPARSE):
    circuit = [C(0,1,Ec,'Cap')]
    circuit += [J(0,1,Ej,'JJ')]
...
def shuntedQubit(Rep,basis,josephson=[120.,50,120.],cap=[10.,50.,10.],ind=100.,sparse=SPARSE,symmetry=False,_C_=(C_,C0),_J_=(J_,J0)):
...
def phaseSlip(Rep,basis,sparse=SPARSE,inductance=[.001,.0005,.00002,.00035,.0005],capacitance=[100,30,30,30,30,40,10]):
